...
@section('header')
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center">
        <div>
            <h1 class="text-3xl font-bold text-neutral-900">Upcoming Events</h1>
            <p class="mt-2 text-sm text-neutral-600">Discover amazing events and book your tickets</p>
        </div>
        @auth
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        @if($events->count() > 0)
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach ($events as $event)
                    <div class="bg-white rounded-xl shadow-sm hover:shadow-lg transition-shadow duration-300 overflow-hidden border border-neutral-200">
                        <!-- Event Image Placeholder -->
                        <div class="h-48 bg-gradient-to-r from-primary-500 to-accent-600 relative">
                        ...
                        <div class="p-6">
                            ...
                            @if($event->description)
                                <p class="text-neutral-600 text-sm mb-4 line-clamp-3">{{ $event->description }}</p>
                            @endif
                            ...
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <div class="text-right">
                                    <div class="text-lg font-bold text-neutral-900">RM {{ number_format($event->ticket_price, 2) }}</div>
                                    <div class="text-xs text-neutral-500">per ticket</div>
                                </div>
                            </div>
                            ...
                            <div class="flex space-x-2">
                                <a href="{{ route('events.show', $event) }}"
                                   class="flex-1 bg-primary-600 hover:bg-primary-700 text-white text-center py-2 px-4 rounded-lg text-sm font-medium transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2">
                                    View Details
                                </a>
                                @auth
                                    @if(auth()->user()->isAdmin())
                                        <div class="flex space-x-1">
                                            <a href="{{ route('events.edit', $event) }}"
                                               class="p-2 text-neutral-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors duration-200"
                                               title="Edit Event">
                                            ...
                                            <form action="{{ route('events.destroy', $event) }}" method="POST" class="inline"
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                        class="p-2 text-neutral-400 hover:text-danger-600 hover:bg-danger-50 rounded-lg transition-colors duration-200"
                                                        title="Delete Event">
                                                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @else
            ...
            <div class="text-center py-12">
                ...
                <h3 class="mt-4 text-lg font-medium text-neutral-900">No events found</h3>
